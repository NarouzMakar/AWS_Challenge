...
namespace narouz_makar_aws_challenge
{
    public class UploaderHelper
    {
        ...
        static void UploadFile(string s3BucketName, string fileName, string contentType, string contentBody)
        {
            try
            {
                ...
                string key = Environment.GetEnvironmentVariable("Access_key_ID");
                string secretKey = Environment.GetEnvironmentVariable("Secret_Access_Key");

                var awsCredentials = new Amazon.Runtime.BasicAWSCredentials(key, secretKey);
